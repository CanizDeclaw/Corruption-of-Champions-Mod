...
{
    class BeginCharacterCreation : CommonScene
    {
        ...
        private readonly Dictionary<string, SpecialPlayerCharacter> specialCharacters;

        public BeginCharacterCreation(Game game)
            : base(game)
        {
            #region UI Hints
            ...
            specialCharacters = new SpecialPCsList(game).GetSpecialCharactersDictionary();
            Commands[0] = new UseNormalName(game, specialCharacters, nameBoxKey, comboBoxKey);
            Commands[0].ChangeTextBoxEvents[nameBoxKey] += (string name) => UpdateSpecialCharDescription(name); // We only need this once.
            Commands[1] = new UseSpecialName(game, specialCharacters, nameBoxKey, comboBoxKey);
            SetDescription();
        }
